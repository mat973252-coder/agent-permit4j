...
public final class DecisionPipeline {
  ...
  public DecisionResult process(ToolInvocation invocation) {
    ...
    return decide(normalized, risk);
  }

  private DecisionResult decide(ToolInvocation invocation, RiskAssessment risk) {
    return switch (risk.level()) {
      case LOW -> execute(invocation, risk.reasonCode());
      case HIGH, CRITICAL ->
          finish(invocation, DecisionOutcome.APPROVAL_REQUIRED, risk.reasonCode());
