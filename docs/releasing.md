...
The prepared release version is `0.5.0`; per the [release record](iterations/v0.5-release.md)
it was validated and published to Maven Central on 2026-09-22, and the released
coordinates were consumed from an empty Maven repository the same day. Current
public availability beyond that dated evidence has not been re-verified here.
...
A `status` job runs after `verify` and `adoption` (`if: always()`) and exports a
`mat-console.status/1` JSON document from the actual job results of the same
workflow run. It is uploaded as the short-lived `mat-console-status` artifact.
The document is generated by `scripts/export_status.py` (standard library only,
no credentials) and reports only allowlisted facts: run identity, job results,
milestone states and evidence links; `generated_at` is when the document was
produced, not when the underlying evidence was verified. The behavioral mapping
is covered by `scripts/test_export_status.py`; generate or test locally with:

```bash
python3 scripts/export_status.py --server-url https://github.com \
  --repository <owner>/<repo> --run-id <id> --run-number <n> --sha <commit> \
  --verify-result success --adoption-result success --output status.json
